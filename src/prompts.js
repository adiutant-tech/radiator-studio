...
// Od v6.5 swap zaworów NIE dostaje zdjęcia referencyjnego zaworu: design jest
// już w kadrze, a referencja w innym metalu tylko re-forsowała swój materiał
// (Nano Banana Pro rozstrzyga konflikt tekst-obraz na korzyść obrazu).
export const SWAP_REF_SINGLE = `Both valves keep exactly the design they already have in this image: the same body shape, handwheel, spindle and fittings, in the same positions. Only the metal changes.`

export const SWAP_REF_PAIR = `Each valve keeps exactly the design it already has in this image, on its own side: the handwheel valve stays on the left, the plain lockshield stays on the right. Do not swap the two designs between sides and do not make them match. Only the metal changes.`
